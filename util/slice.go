...
// SliceContainInt 判断某个int值是否在切片中
func SliceContainInt(finder int, slice []int) bool {
	exists := false
	for _, v := range slice {
		if v == finder {
			exists = true
...
// SliceShuffleInt 打乱一个切片
func SliceShuffleInt(src []int) []int {
...
// SliceIsSame 判断两个slice是否相等
func SliceIsSame(slice1, slice2 []int) bool {
...
// SliceContainString 判断某个string值是否在切片中
func SliceContainString(finder string, slice []string) bool {
	exists := false
	for _, v := range slice {
		if v == finder {
			exists = true
